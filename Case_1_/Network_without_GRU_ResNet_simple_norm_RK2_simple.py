...
class topDNN(nn.Module):
    def __init__(self,layers,lastbias = False,use_batchnorm = True):
        ...
        for i in range(self.depth - 1):
            if use_batchnorm == True:
                layer_list.append((f'batchnorm_{i}', nn.BatchNorm1d(layers[i],eps = 1e-5,affine = False,momentum=0.1,track_running_stats=True)))  ###这里两字符串和modulus命令放在一起是一个二元组
            layer_list.append((f'mlp_layer_{i}', nn.Linear(layers[i], layers[i + 1])))
            layer_list.append((f'activation_{i}', self.activation()))        # layers[i + 1]   
    ...
    def forward(self, x):
        out = x  # Initialize out with the input x
        for name, layer in self.layers.named_children():
            if 'batchnorm_' in name:
                batchnorm_out = layer(out)  # Process input through Batchnorm layer
            elif 'mlp_layer_' in name:
                mlp_out = layer(batchnorm_out)  # Process input through MLP layer
            elif 'activation_' in name:
                out = layer(mlp_out)  # Process MLP output through activation          
            elif 'output_layer' in name:
                out = layer(out)  # Final output layer processing
